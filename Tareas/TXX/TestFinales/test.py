...
if __name__ == "__main__":
    alumnos = os.listdir("TXX")
    alumnos_result = {}
    ignorar = ['NatachaP', 'aescobar4', 'robertov1', 'ByronHidalgo1']
    # ignorar = []
    # alumnos = ['NatachaP', 'ByronHidalgo1']
    # alumnos = [ 'aescobar4']
    for alumno in alumnos:
        ...
        try:
            os.remove("main.py")
            os.remove("tree.py")
            os.removedirs('student_solution')
            os.makedirs('student_solution')
        except Exception:
            pass
        ...
        if alumno in ignorar:
            continue

        if alumno == ".DS_Store":
            continue
        path_alumno = join("TXX", alumno)

        

        with open(join(path_alumno, "main.py")) as file:
            with open("main.py", "w") as file_2:
                for line in file:
                    file_2.write(line)
        
        with open(join(path_alumno, "tree.py")) as file:
            with open("tree.py", "w") as file_2:
                for line in file:
                    file_2.write(line)
        ...
        # if 'deque' in vars() or 'collections' in vars():
        #     try:
        #         del vars()['deque']
        #         del vars()['collections']
        #     except:
        #         pass
        #     alumnos_result[alumno] = ",".join(["import collections"]*30)
        #     continue

        # #exec("".join(open("main.py").readlines()))
        # #exec("".join(open("tree.py").readlines()))
        ...
        alumnos_result[alumno] = ",".join([str(x) for x in text])

    for e in alumnos_result:
        print("{},{}".format(e, alumnos_result[e]))